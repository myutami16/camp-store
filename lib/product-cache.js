const callRevalidateAPI = async (tags = [], paths = []) => {
	try {
		...
		// Debug token
		const token = "adzracamp-revalidate-token-2025"; // Hardcode untuk testing
		...
		console.log(
			"🔍 All env vars related to REVALIDATE:",
			Object.keys(process.env).filter((key) => key.includes("REVALIDATE"))
		);

		console.log("🔍 About to send request with headers:", {
